...
const files = cp.execSync('git ls-files mobile dashboard data docs/yandex', { cwd: ROOT }).toString().trim().split('\n');
const RX = /^(<{7}( |$)|={7}$|>{7}( |$))/m;
let bad = 0;
for (const f of files) {
  if (!/\.(html|js|cjs|json|css|md)$/.test(f)) continue;
  const s = fs.readFileSync(path.join(ROOT, f), 'utf8');
  const m = RX.exec(s);
  if (m) { bad++; const line = s.slice(0, m.index).split('\n').length;
    console.log('  FAIL  ' + f + ':' + line + ' carries a conflict marker   ' + JSON.stringify(m[0])); }
}
console.log((bad ? '  FAIL  ' : '  PASS  ') + 'no shipped file carries a git conflict marker   ' + files.length + ' files read');
