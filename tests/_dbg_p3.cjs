...
(async () => {
  ...
  if (hasPhoto) {
    const who = await p.evaluate(()=>new Promise(res=>{
      const orig = window.renderHistory; const stacks=[]; let n=0;
      window.renderHistory = function(){ n++; if(stacks.length<3){ try{ throw new Error("x"); }catch(e){ stacks.push(e.stack.split("\n").slice(1,7).join(" | ")); } } return orig.apply(this,arguments); };
      setTimeout(()=>{ window.renderHistory=orig; res({n,stacks}); }, 1500);
    }));
    console.error("DBGWHO2", JSON.stringify(who,null,1));
