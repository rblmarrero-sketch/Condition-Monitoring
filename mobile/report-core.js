...
/* ---- an earlier round, as it was reported ---------------------------------
   A header of one line and a strip of cards. The whole block is about a fifth
   of a sheet, which is what puts four or five rounds on one page — and it is
   the same thing the office screen shows, so a reader moving between them is
   not learning two layouts for one fact. */
#rptRoot .sec.olderr{margin-top:9px;border-top:1px solid #dfe4e9;padding-top:6px;}
#rptRoot .ohd{display:flex;align-items:baseline;gap:10px;flex-wrap:wrap;}
#rptRoot .ohd b{font-size:13px;font-weight:800;font-variant-numeric:tabular-nums;
  letter-spacing:-.01em;}
#rptRoot .ohd > span{font-size:10px;color:#5b6670;}
#rptRoot .ohd .oalt{color:#8b939b;margin-left:4px;}

#rptRoot .ohd .ov{margin-left:auto;}
/* The cards, shrunk. Every element keeps its job and loses its padding: the
   photograph stays the biggest thing on the card, because it is what the
   reader came for. */
#rptRoot .board.mini{gap:6px 6px;margin-top:6px;}
...
`;
  ...
  var S = {
    en: {
      ...
      /* The same idea for a round recorded in grades rather than millimetres.
         ...
         one point read across the visits, and the heading should say so. */ c_chg:"Change", c_now:"Latest", c_limit:"New → condemn",
    ...
    ru: {
      ...
      trend_t:"История замеров", trend_sub:"строка на точку, слева — самый ранний замер", c_chg:"Изменение", c_now:"Текущий",
  ...
  function makeT(lang, bi) {
    ...
    T.both = function (a, b, cls) {
      if (!bi || !b || b === a) return esc(a);
      /* The block classes put the translation on its own line and need no
         separator. An INLINE one does: without it a header printed "Magnetic
  ...
  /* ---- the earlier rounds, as they were reported ---------------------------
     The same thing the office screen shows: the photographs, and under each one
     what that point was. Nothing else.

     There were tables here — a line per round, and a grid of every point
     against every round — and they were struck out on the returned sheet with
     one word: redundant. They were. Every fact in them is on a card: the
     grade, the severity, the reading, the finding, the date and the hour meter
     in the header above them. Three renderings of one truth is not
     thoroughness, it is three places for them to disagree.

     COMPACT, because the ask was four or five rounds to a page and a page is
     744 points tall. A round is a header of about eighteen and a strip of
     cards; at four across the cards come out near 130 points, which puts five
     rounds and the masthead on one sheet with room to spare. The photograph is
     still the biggest thing on the card, because it is what the reader came
     for.

     What is NOT reprinted is the furniture: one masthead, one drawing of the
     machine, one signature block per document. That was the point of
     collapsing the rounds in the first place, and none of it comes back.
  ...
  function earlierRoundSections(ctx, T, older) {
    ...
    older.forEach(function (rec) {
      ...
      /* A measured round's record is its millimetres, and those are lined up in
         the table above where they can be compared. Its photographs are still
         evidence. */
      if (rec.wear) told = told.filter(function (it) { return it.photos && it.photos.length; });
      /* A round where every point was fine and nobody photographed anything has
         nothing to show, and a header with no evidence under it is a section
         that exists to hold its own heading. */
      ...
      out.push({ nb: false, gap: 4, html: '<div class="sec olderr">'
        + '<div class="ohd">'
          + '<b>' + esc(rec.date || "") + '</b>'
          /* An explicit separator, not a styled span. T.both's block variants
             put the translation on its own line, which is right in a table
             cell and wrong on a header line — it printed "Magnetic
             PlugМагнитная пробка", two words run together with nothing
             between them. A pseudo-element would be the tidy fix and
             html2canvas does not reliably paint one. */
          + '<span>' + T.both(rec.typeLabel || rec.type, rec.typeAlt, "oalt") + '</span>'
          + ((rec.smu || rec.by) ? '<span>'
  ...
  /* ---- the millimetres, lined up ------------------------------------------
     One row per measured point, one column per round, oldest reading on the
     left, so a part walking towards its limit shows as a line of numbers going
     one way. Beside them: the limit it is walking towards, the change over the
     whole series, the percentage worn and the hours left on it.

     This is the ONE table left in a unit report. There were two more — a line
     per round, and the same point-per-row grid in grades — and once every
     earlier round prints with its photographs and its grades on each card,
     both of them said a third time what the cards already said twice.

     Not this one. Nothing on a card carries a condemn limit, a rate of wear or
     a forecast, and a reader cannot reconstruct any of them by looking. A
     table nobody can rebuild from the pictures is not redundant. */
  function wearHistorySections(ctx, T, latest, older) {
    var out = [];
  ...
  function commonBand(T, sh, n, oneLine) {
    if (!sh.defect && !sh.cause && !sh.action) return "";
    /* On an earlier round the same three facts as a definition list are 67
       points of a block that has to fit five to a page. Same words, one line,
       no labels — a reader who has seen the band above on the latest round
  ...
  CMR.paginate = async function (opts) {
    ...
    try{
      ...
      for(var i=0;i<els.length;i++){
        ...
        /* The gap AFTER a section, and it was a literal 14 for every one of
           them. An earlier round already carries its own rule and margin, so
           fourteen more points on top is a sixth of a block, and across five
           blocks it is the difference between four to a page and five — which
           is what was actually asked for. A section may say what it needs. */
        y += (opts.sections[i].gap != null ? opts.sections[i].gap : 14);
      }
